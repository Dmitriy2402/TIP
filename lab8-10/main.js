...
removeButton.addEventListener("click", () => {
  ...
  localStorage.clear();
  if (
    document.querySelector(".month").contains(document.querySelector(".add"))
  ) {
    document
      .querySelector(".month")
  ...
  if (
    document.querySelector(".quarter").contains(document.querySelector(".add"))
  ) {
    document
...
addButton.addEventListener("click", () => {
  if (lastname.value != "" && date.value !== "") {
    i = +localStorage.getItem("count"); //  это свойство глобального объекта
    //браузера (window).получение по ключу значения Объекты веб-хранилища localStorage и sessionStorage позволяют хранить пары
//ключ/значение в браузере. 
    if (+document.querySelector(".lastname").textContent === 0) {
      tbody.innerHTML = "";
    }
    document.querySelector("form").classList.remove("form_required"); // удаление после добавления
    const obj = {
      toString() {
        return `<tr><td class="lastname">${lastname.value}</td><td class="date">${date.value}</td></tr>`;
      },
    };
    localStorage.setItem(`${i}`, obj); // cохранение строки таблицы в local storage 
...
if (localStorage.getItem("0") !== null) { // получить данные по ключу key
  // добавление из local storage
...
document.querySelector("#count").addEventListener("click", () => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    <td class="start-date">${start.getFullYear()}-${
      start.getMonth() + 1 < 10
        ? "0" + (start.getMonth() + 1)
        : start.getMonth() + 1
    }-${start.getDate() < 10 ? "0" + start.getDate() : start.getDate()}</td>
    <td class="end-date">${end.getFullYear()}-${
      end.getMonth() + 1 < 10 ? "0" + (end.getMonth() + 1) : end.getMonth() + 1
    }-${end.getDate() < 10 ? "0" + end.getDate() : end.getDate()}</td></tr>`;
  }
});
...
function show() {
  ...
  if (array.length === 0) {
    p.innerHTML =
      "В этом месяце никому из сотрудников отпуск не предоставляется";
  } else {
    p.innerHTML = array;
  }
...
function oneShow() {
  ...
  if (array.length === 0) {
    p.innerHTML =
      "В этом квартале никому из сотрудников отпуск не предоставляется";
  } else {
    p.innerHTML = array;
  }
...
function twoShow() {
  ...
  p.className = "add";
  if (
    document.querySelector(".year").contains(document.querySelector(".add"))
  ) {
  ...
  if (array.length === 0) {
    p.innerHTML =
      "В следующем году никому из сотрудников отпуск не предоставляется";
  } else {
    p.innerHTML = array;
  }
